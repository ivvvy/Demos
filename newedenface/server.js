...
mongoose.connection.on('error',function(){
   console.info('Error£ºCould not connect to MongoDB.Did you forget to run "mongod"?');
});
 ...
 */
app.get('/api/charaters',function(req,res,next){
    ...
    Character.find({random:{$near:[Math.random(),0]}})
        ...
        .exec(function(err,characters){
            ...
            var oppositeGender=_.first(_.window(choices,randomGender));
...
app.put('/api/characters',function(req,res,next){
    ...
    async.parallel([
        ...
        function(err,results){
            ...
            if(!winner || !loser){
                return res.status(404).send({message:'One of character no longer exists.'})
            }
